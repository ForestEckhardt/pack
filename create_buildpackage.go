...
type CreateBuildpackageOptions struct {
	Name    string
	Config  buildpackage.Config
	Publish bool
}

func (c *Client) CreateBuildpackage(ctx context.Context, opts CreateBuildpackageOptions) error {
	...
	tmpDir, err := ioutil.TempDir("", "create-buildpackage")
	if err != nil {
		return err
	}

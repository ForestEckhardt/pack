...
func testAcceptance(t *testing.T, when spec.G, it spec.S, builder, runImageMirror, packFixturesDir, packPath string, lifecycleDescriptor builder.LifecycleDescriptor) {
	...
	when("create-buildpackage", func() {
		var tmpDir string

		it.Before(func() {
			h.SkipIf(t, !packSupports(packPath, "create-buildpackage"), "pack does not support 'create-buildpackage'")

			var err error
			tmpDir, err = ioutil.TempDir("", "create-buildpackage-tests")
			h.AssertNil(t, err)
		...
		it("creates the buildpackage", func() {
			buildpackageName := "test/buildpackage-" + h.RandString(10)
			...
			h.AssertNil(t, err)
			h.AssertContains(t, output, fmt.Sprintf("Successfully created buildpackage '%s'", buildpackageName))

			_, _, err = dockerCli.ImageInspectWithRaw(context.Background(), buildpackageName)
			h.AssertNil(t, err)
		})

		when("--publish", func() {
			it("publishes image to registry", func() {
				buildpackageName := registryConfig.RepoName("test/buildpackage-" + h.RandString(10))
				...
				h.AssertNil(t, err)
				h.AssertContains(t, output, fmt.Sprintf("Successfully published buildpackage '%s'", buildpackageName))

				_, _, err = dockerCli.ImageInspectWithRaw(context.Background(), buildpackageName)
				h.AssertError(t, err, "No such image")

				h.AssertNil(t, h.PullImageWithAuth(dockerCli, buildpackageName, registryConfig.RegistryAuth()))
